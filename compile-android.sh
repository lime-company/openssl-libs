...
# juraj's changes:
#   - temporary folder is located in the current folder
#	- the script doesn't perform make install, because it's too slow. we're just
#     copying final libraries and headers to the destination folder
#	- make -j{concurent} to speedup the build
...
OPENSSL_TMP_FOLDER="$CURRENT_PATH/.openssl-tmp"
...
function build_library {
	...
	# juraj: install is too slow. Now we're only performing make & copy headers and final libraries
    #make && make install

	CONCURENCY=$(($(getconf _NPROCESSORS_ONLN) + 2))

	make -j${CONCURENCY}
	mkdir -p "${OPENSSL_OUTPUT_PATH}/include"
	mkdir -p "${OPENSSL_OUTPUT_PATH}/lib/${OPENSSL_TARGET_ABI}"
	cp -LR "${OPENSSL_TMP_FOLDER}/include/openssl"	"${OPENSSL_OUTPUT_PATH}/include"
	cp      ${OPENSSL_TMP_FOLDER}/lib*.a			"${OPENSSL_OUTPUT_PATH}/lib/${OPENSSL_TARGET_ABI}"
	rm -rf "$OPENSSL_TMP_FOLDER"
